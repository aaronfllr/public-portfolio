...
export function BlogCard({ id, title, content, imagePath }: BlogCardProps) {
  return (
    <Card className="flex shadow-md shadow-hit-pink-800 overflow-hidden flex-col">
      ...
      <CardFooter>
        <Button asChild size="lg" className="bg-hit-pink-950 w-full text-hit-pink-300 hover:bg-hit-pink-900">
          <Link href={`/blog/${id}/view`}>Read More</Link>
        </Button>
      </CardFooter>
    </Card>
  );
